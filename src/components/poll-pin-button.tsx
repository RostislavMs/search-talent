...
export default function PollPinButton({
  pollId,
  currentPinnedUntil,
  locale,
}: {
  ...
  const ui =
    locale === "uk"
      ? {
          pin: "Закріпити",
          pinned: "Закріплено",
          title: "Закріпити опитування",
          description: "Опитування буде зверху стрічки протягом обраного терміну.",
          day: "На добу",
          threeDays: "На 3 дні",
          week: "На тиждень",
          unpin: "Відкріпити",
          close: "Закрити",
          saving: "Збереження...",
          error: "Не вдалося закріпити опитування.",
        }
      : {
          pin: "Pin",
          pinned: "Pinned",
          title: "Pin poll",
          description: "The poll will stay at the top of the feed for the chosen duration.",
          day: "24 hours",
          threeDays: "3 days",
          week: "1 week",
          unpin: "Unpin",
          close: "Close",
          saving: "Saving...",
          error: "Could not pin the poll.",
        };

  async function handlePin(duration: PinDuration) {
    setSaving(true);
    setError("");
    const result = await apiFetch(`/api/polls/${pollId}`, {
      method: "PATCH",
  ...
  return (
    <>
      <Button variant={isPinned ? "primary" : "ghost"} onClick={() => setIsOpen(true)}>
        {isPinned ? ui.pinned : ui.pin}
      </Button>

      {isOpen && (
        <div className="fixed inset-0 z-[70] flex items-end bg-black/45 px-4 py-4 sm:items-center sm:px-6">
          <div className="mx-auto w-full max-w-md rounded-hero border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="font-display text-2xl font-medium tracking-tight text-[color:var(--foreground)]">
                  {ui.title}
                </h2>
                <p className="mt-3 text-sm leading-7 app-muted">{ui.description}</p>
              </div>
