...
/**
 * Moderation list over the `articles` table, shared by the Articles and
 * Discussions admin sections. The two differ only in which slice they cover and
 * where their rows and links point — everything else (status chips, search,
 * bulk actions, pagination) is identical, so they render the same view rather
 * than two copies that drift apart.
 */
export default async function AdminArticlesSection({
  locale,
  searchParams,
  scope,
  basePath,
  ...
  /** Admin route this section lives at, e.g. "/admin/content/discussions". */
  basePath: string;
  /** Public route its rows open, e.g. "/discussions". */
  ...
  const pageParam = Number.parseInt(firstValue(searchParams.page) || "1", 10);
  ...
  function buildPageHref(nextPage: number) {
    ...
    const path = createLocalePath(locale, basePath);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-none sm:rounded-hero app-card p-5 sm:p-8">
        ...
        <p className="mt-2 max-w-3xl app-muted">{description}</p>
      ...
      <section className="rounded-none sm:rounded-hero app-card p-4 sm:p-6">
        ...
        {result.items.length === 0 ? (
          ...
          <ArticlesTableClient
            ...
            locale={locale}
            redirectAfterDelete={createLocalePath(locale, basePath)}
            bulkLabels={{
